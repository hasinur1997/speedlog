...
- **Files touched:** `speedlog/logging_setup.py` (implemented), `tests/test_logging_setup.py`
  (new), `pyproject.toml` (Black target version pinned to py311 so repo checks stay green
  under Python 3.12), `docs/progress-tracker.md` (status update).
- **Logging behavior:** `configure(debug: bool)` adds one named rotating file handler on the
  root logger writing to `config.log_dir() / "app.log"` with 5 MiB rotation and 3 backups.
  A named console handler is added only when `debug=True` or `NST_DEBUG=1`.
...
- **Tests:** 5 new unit tests cover file handler configuration, file output/format content,
  ...
  logging. Verified with `pytest -q`, `ruff check .`, and `black --check .`.
